    ...
    for (iz, z) in enumerate(z_vec)
        ...
        for (ia_next, a_next) in enumerate(a_vec)
            for iz_next in 1:N_z
                ...
                # Consumption tomorrow (net of lump-sum tax)
        ...
        if sum(valid) >= 2
            ...
            for (ia, a) in enumerate(a_vec)
                ...
                # Ensure feasibility: can't save more than total resources (net of tax)
                max_saving = (1+r)*a + w*z - T - 1e-10  # leave tiny bit for consumption
